## 1. Основные компоненты и блоки модели

### 1.1. Введение
Согласно варианту задания, необходимо разработать имитационную модель процесса передачи live-стрима на платформе Twitch. Основной акцент делается на нелинейной динамике системы: зависимости качества восприятия (аудио/видео синхронизация, рывки) от состояния сети и настроек буферизации.

Модель включает следующие функциональные блоки: **Генерация контента (Стример)**, **Кодирование/Отправка**, **Сетевой канал доставки (CDN/Интернет)**, **Клиентский буфер (Плеер зрителя)**, **Декодирование и Воспроизведение (Зритель)**.

#### Законы моделирования внутри блоков:
...
##### 3. Блок «Сетевой канал доставки (Интернет/CDN)»
...
**Процесс 1: Кодирование и отправка (Стример)**
*   **Входные данные:** Сырой видеопоток (картинка) и аудиопоток (звук).
*   **Процесс:**
    1.  Кадры захватываются с постоянным интервалом (T_video = 1/FPS).
    2.  Кодек анализирует кадр. Если кадр сложный (много движения, смена сцены), размер сжатого кадра (в байтах) получается больше, и время на его кодирование увеличивается.
    3.  Крупный кадр фрагментируется на несколько сетевых пакетов, которые отправляются в сеть пачкой (burst).
    4.  Аудио кодируется независимо, но пакеты аудио отправляются с меньшим интервалом, но постоянным размером.
*   **Выходные данные:** Последовательность пакетов с метками времени (PTS - Presentation Timestamp) и размером в байтах.

**Процесс 2: Транспортировка по сети**
*   **Входные данные:** Пакеты от стримера, поступающие в определенные моменты времени.
*   **Процесс:**
    1.  Пакет попадает в очередь на отправку (буфер маршрутизатора).
    2.  Скорость отправки пакетов ограничена пропускной способностью канала (bandwidth).
    3.  Если пакетов в очереди слишком много и новый пакет приходит в переполненный буфер — он отбрасывается (потеря).
    4.  Каждый пакет задерживается на время, равное сумме времени ожидания в очереди (Q_delay) и времени распространения (propagation delay, константа).
    5.  Джиттер (вариация задержки) возникает из-за того, что разные пакеты застают разную длину очереди.
*   **Выходные данные:** Модифицированный поток пакетов с новыми временами получения (client_arrival_time), возможны пропуски в нумерации пакетов.

**Процесс 3: Буферизация и синхронизация (Плеер)**
*   **Входные данные:** Неравномерный поток пакетов из сети.
*   **Процесс:**
    1.  Пакеты складируются в буфер.
    2.  Плеер имеет внутренние часы (Clock). Он пытается воспроизводить контент с оригинальной скоростью (например, 60 кадров/с).
    3.  **Логика принятия решения:**
        *   Если буфер пуст (нет пакета с текущей меткой времени) -> **Рывок**: воспроизведение останавливается, часы плеера замирают. Счетчик рассинхронизации не увеличивается, так как ничего не играет.
        *   Если в буфере есть и аудио, и видео пакеты:
            *   Сравниваются метки времени аудио (A_pts) и видео (V_pts) для текущего момента.
            *   Если разница |A_pts - V_pts| превышает порог (например, 40 мс), система считает, что наступила **рассинхронизация**.
            *   Коррекция: плеер может временно ускорить воспроизведение видео (пропуская кадры) или аудио (изменяя частоту дискретизации без изменения тона — сложный алгоритм, в модели может быть заменен на "сброс" синхронизации по ключевому кадру), чтобы "догнать" отстающий поток.
    4.  Уровень буфера колеблется: при быстрой сети он растет, при медленной — падает. Это нелинейный процесс, так как скорость наполнения зависит от джиттера и пачечной природы трафика.
*   **Выходные данные:** События для статистики: момент рывка (stall), длительность рассинхронизации, текущий уровень буфера.

### 1.4. Используемые распределения
